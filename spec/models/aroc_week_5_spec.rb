...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select('users.name name, count(orders.id) total_order_count').joins(:orders).group('users.id').order('total_order_count')
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select('users.name, COUNT(order_items.item_id) total_item_count').joins(:order_items).group('users.id').order(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.select('users.name user_name, orders.id order_id, floor(orders.amount/count(order_items.id)) avg_item_cost').joins(:order_items).group('users.name, orders.id').order('user_name desc, avg_item_cost asc')
    # ---------------------------------------------------------------
